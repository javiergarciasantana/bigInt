...
class rpn_base {
 ...
 private:
  std::vector<BigInt<Base>> integers_;//aqui almacenamos los integers que se definen en el fichero de texto
  std::vector<BigInt<Base>> results_;
  std::stack<BigInt<Base>> stack_;//aqui metemos los inte operandos para trabajar con ellos
};
...
void rpn_base<Base>::Operate(std::string& line, int identifier) {
  ...
  std::cout << "E" << identifier << " = " << result << std::endl;
}
